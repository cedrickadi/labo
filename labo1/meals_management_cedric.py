...
def fichier_pdf():
    fichier = "ticket_de_caise.pdf"
    can = Canvas("{0}".format(fichier), pagesize=A4)

    texteA = "ABC inc."
    can.setFont("Times-Roman", 16)
    posX,posY =2.5*cm, 27*cm
    can.drawString(posX, posY,texteA)

    texteB = "Waterloo street, 1 "
    can.setFont("Times-Roman", 16)
    posX,posY =2.5*cm, 26*cm
    can.drawString(posX, posY,texteB)

    texteC = "1000 Brussels"
    can.setFont("Times-Roman", 16)
    posX,posY =2.5*cm, 25*cm
    can.drawString(posX, posY,texteC)

def calcul_tva():
    tva = 0.06
    somme = amount
    valeur_tva = somme*tva
    somme_sans_tva = somme - valeur_tva
    print("    " + "excl.VAT" + "VAT" + "incl. VAT")
    print("VAT 6%" +"{1}"+"{2}"+"{3}", somme, valeur_tva, somme_sans_tva )
